...
import { Activity, AlertTriangle, Clock, User, Filter, RefreshCw } from 'lucide-react'
...
function LogBadge({ type, action }) {
  ...
  // activity badge color by action group
...
export default function Logs() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {isDemo && (
        <div className="card p-4 bg-brand-yellow-light border border-brand-yellow">
          <p className="text-sm text-brand-yellow-dark font-medium">
            Modo demo — os logs abaixo são apenas de exemplo. Em produção, todos os eventos são gravados no Firestore.
          </p>
        </div>
      )}
      ...
      <p className="text-xs text-center text-brand-gray-light pb-4">
        Exibindo os últimos {filtered.length} de {logs.length} registros
      </p>
    </div>
  )
}
